...
/**
 * Get all NFTs minted by a candy machine.
 * @param {Connection} conn - Solana web3 connection.
 * @param {string} candyMachineId - Candy machine address.
 * @param {number} candyMachineVersion - Candy machine version.
 */

async function getMintHashesFromCandyMachine (conn, candyMachineId, candyMachineVersion) {
  const metaplex = new Metaplex(conn);
  const candyMachine = new PublicKey(candyMachineId);
  const mintedNfts = await metaplex.candyMachines().findMintedNfts({ candyMachine, version: candyMachineVersion }).run();

  let mints = [];
  
  for(const mint of mintedNfts) {
    let mintAddr = mint.mintAddress.toBase58();
    mints.push(mintAddr);
  }

  return mints;
}
exports.getMintHashesFromCandyMachine = getMintHashesFromCandyMachine;
 ...
 *  
 * @param {string} addr - Solana address.
 * 
*/
